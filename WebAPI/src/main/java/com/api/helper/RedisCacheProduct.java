...
public class RedisCacheProduct {

	public List<ProductModel> GetListDataFromCache(ProductDAO proDAO, RedisTemplate redisTemp, String KeyMap){	
		List<ProductModel> lstPro = null;
		
		if(redisTemp.hasKey("CacheMapData"))  //Nếu đã có Cache1 thì kiểm tra Map lưu trong cache1
		{
			Map<String, List<ProductModel>> map1 = new HashMap<String, List<ProductModel>>();
			map1 = (Map<String, List<ProductModel>>) redisTemp.opsForValue().get("CacheMapData"); //lấy Map lưu trong Cache
			//Xem trong Map đã có lưu key: data cần lấy chưa
			if(map1.get(KeyMap) != null) //nếu có data thì lấy ra lun
			{
				lstPro = map1.get(KeyMap);
			}
